...
### Q: 稀疏索引仍然出现null值冲突？
A: **v0.1.5版本最终解决方案** - MongoDB稀疏索引对null值的唯一性约束问题：
1. **问题根源**：MongoDB稀疏索引虽然忽略缺失字段，但对存在的null值仍会应用唯一性约束
2. **最终修复**：
   - 移除身份证号的唯一索引约束：`POST /api/debug/remove-idcard-index`
   - 改为应用层检查：只对非空身份证号进行重复性验证
   - 创建普通索引提升查询性能，但不强制唯一性
3. **修复效果**：✅ 支持创建无限个空身份证号的招聘记录
4. **验证方法**：可连续创建多个身份证号为空的不同招聘记录，不再报错
